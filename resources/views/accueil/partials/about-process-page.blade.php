...
<div class="process process--page" id="process">
  @if($steps->isEmpty())
          ...
          <article class="process-timeline-item reveal active">
            ...
            <div class="process-timeline-card">
              <div class="process-timeline-card-top">
                <div class="process-timeline-icon"><i class="fas {{ $icon }}" aria-hidden="true"></i></div>
  ...
  (function initProcessStepDetailModal() {
    ...
    function closeModal() {
      ...
      window.setTimeout(() => {
        ...
        document.body.classList.remove('service-section-modal-open');
    ...
    document.addEventListener('click', (event) => {
      ...
      if (!trigger) {
        if (event.target.closest('[data-process-step-detail-close]')) closeModal();
        return;
      }

      event.preventDefault();
      lastFocused = document.activeElement;
      const source = trigger.parentElement?.querySelector('[data-process-step-detail-html]');
      if (titleEl) titleEl.textContent = trigger.dataset.stepTitle || 'Étape';
      if (indexEl) indexEl.textContent = String(trigger.dataset.stepIndex || '').padStart(2, '0');
      if (contentEl) contentEl.innerHTML = source ? source.innerHTML : '';
      modal.hidden = false;
      modal.setAttribute('aria-hidden', 'false');
      document.body.classList.add('service-section-modal-open');
