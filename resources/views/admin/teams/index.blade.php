...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            @can('create_users')
            <a href="{{ route('admin.teams.create') }}" class="btn-primary inline-flex items-center w-full sm:w-auto justify-center">
                    ...
                    <table class="min-w-full divide-y divide-border">
                        <thead class="bg-neutral-100">
                            <tr>
                                ...
                                <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-primary uppercase tracking-wider hidden md:table-cell">Rôles</th>
                                <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-primary uppercase tracking-wider hidden lg:table-cell">Groupe</th>
                            ...
                            @forelse($users as $user)
                                <tr class="hover:bg-neutral-100/50 transition-colors">
                                    ...
                                    <td class="px-3 sm:px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                                        <span class="px-2 py-1 text-sm text-secondary bg-neutral-100 rounded-md">
                                            {{ $user->groupe->nom ?? '-' }}
                                        </span>
                                    </td>
                                    <td class="px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('admin.teams.show', $user) }}" class="text-primary hover:text-primary/80 transition-colors" title="Voir">
                                            ...
                                            @can('edit_users')
                                            <a href="{{ route('admin.teams.edit', $user) }}" class="text-warning hover:text-warning/80 transition-colors" title="Modifier">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-secondary text-base font-medium">Aucun membre d'équipe trouvé</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div class="bg-white rounded-2xl max-w-sm sm:max-w-md w-full relative z-10 overflow-hidden border border-gray-100" style="box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 0 0 1px rgba(0, 0, 0, 0.05);">
            <div class="px-4 py-5 sm:px-6 sm:py-6">
                ...
                <p class="text-secondary text-sm sm:text-base mb-6">
                    Êtes-vous sûr de vouloir supprimer le membre d'équipe <strong x-text="userToDelete"></strong> ? Cette action est irréversible.
                </p>
