            ...
            @foreach($blocks as $block)
                <div x-show="activeTab === '{{ $block->key }}'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                    <div class="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <form action="{{ route('admin.vitrine.update', $block) }}" method="POST" class="p-4 sm:p-6" @if(in_array($block->key, ['hero', 'gallery', 'services', 'academy', 'header', 'footer'])) enctype="multipart/form-data" @endif @if($block->key === 'academy') x-data="{ submitting: false }" @submit="submitting = true" @endif>
                            @csrf
                            @method('PATCH')
