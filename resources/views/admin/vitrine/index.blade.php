                    ...
                    @foreach($blocks as $block)
                        ...
                        <div class="{{ $activeTab === $block->key ? '' : 'hidden' }}"
                            ...
                            <div class="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
                                ...
                                @else
                                    <form action="{{ route('admin.vitrine.update', $block) }}" method="POST" class="p-4 sm:p-6" @if(in_array($block->key, ['hero', 'gallery', 'partners', 'temoignages', 'services', 'academy', 'header', 'footer'])) enctype="multipart/form-data" @endif @if($block->key === 'academy') x-data="{ submitting: false }" @submit="submitting = true" @endif>
                                        @csrf
                                        @method('PATCH')
