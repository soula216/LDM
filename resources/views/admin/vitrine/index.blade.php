@php
    $tabIcons = [
        ...
        'temoignages' => 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z',
        ...
        'vos-patients' => 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z',
        ...
        'mentions-legales' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
    ];
@endphp
    ...
    <div x-data="{ activeTab: @js($activeTab) }"
         x-init="
            $watch('activeTab', (tab) => {
                ...
                if (tab === 'mentions-legales') $dispatch('vitrine-mentions-legales-tab-open');
            });
            ...
            if (activeTab === 'mentions-legales') { $nextTick(() => $dispatch('vitrine-mentions-legales-tab-open')); }
         "
                    ...
                    @foreach($blocks as $block)
                        ...
                        <div class="{{ $activeTab === $block->key ? '' : 'hidden' }}"
                            ...
                            <div class="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
                                ...
                                @else
                                    <form action="{{ route('admin.vitrine.update', $block) }}" method="POST" class="p-4 sm:p-6" @if(in_array($block->key, ['hero', 'gallery', 'partners', 'temoignages', 'vos-patients', 'services', 'academy', 'header', 'footer'])) enctype="multipart/form-data" @endif @if($block->key === 'academy') x-data="{ submitting: false }" @submit="submitting = true" @endif>
                                        @csrf
                                        @method('PATCH')
