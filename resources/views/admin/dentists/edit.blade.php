<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-2 sm:space-y-0">
            <div class="flex items-center space-x-3">
                ...
                <h2 class="text-xl sm:text-2xl font-semibold text-primary">
                    {{ __('Modifier Dentiste : ') . $user->full_name }}
                </h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-4 sm:py-8 bg-app min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="card">
                <form action="{{ route('admin.dentists.update', $user) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            ...
                            <x-input id="num_dentist" name="num_dentist" type="number" class="block w-full input-field" value="{{ $user->num_dentist }}" />
                            <x-input-error for="num_dentist" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="order" value="{{ __('Ordre') }}" class="text-primary font-medium mb-2" />
                            <x-input id="order" name="order" type="number" class="block w-full input-field" value="{{ $user->order }}" />
